...
<table>
  ...
  <?php       
              ...
              $use=$_SESSION['userid'];
              $sel="SELECT * FROM giohang where idkh='$use'";
              $r=$con->query($sel);
              $total=0;
              $sl=0;
              while($row=$r->fetch_assoc()){
              $sl=$sl+$row['soluong'];
              $total=$total+$row['giasp']*$row['soluong'];            
  ?>
  <p><input type="hidden" name="ids" value="<?php echo $row['id']?>"></p>
  <tr>
    <th style="padding-left:5px;"><img src="<?php echo $row['hinhanh']; ?>" style="width:90px;border:1px solid white;box-shadow: 1px 1px 2px 2px #ff3300;"></th>
    <th style="padding-left:30px;"><?php echo $row['tensp']; ?></th>
    <th style="padding-left: 25px;"><?php echo $row['giasp']; ?></th>   
    <th style="padding-left: 25px;">x<?php echo $row['soluong']; ?></th>
    <th style="padding-left: 25px;">=<?php  echo $row['soluong']*$row['giasp'];?></th>
...
<article class="col-md-4" style="margin-top: 100px;">
  ...
  <div style="border:2px solid black;padding:10px;display:inline-block;box-shadow: 2px 2px 4px 4px #808080;">
              ...
              <p><input type="hidden" name="tongtien" value="<?php echo $total;?>"></p>
              <p><input type="hidden" name="soluongsp" value="<?php echo $sl;?>"></p>
              <p><input type="hidden" name="idkh" value="<?php echo $use;?>"></p>
              <p>Chọn hình thức thanh toán</p>
              <p><input type="hidden" name="idkh" value="<?php echo $use;?>"></p>
              <input type="radio" name="hinhthuc" value="Tiền mặt">Tiền mặt <br><input type="radio" name="hinhthuc" value="Ví điện tử">Ví điện tử</p>
              </center>
  <p>
                <input type="submit" name="sp" class="btn btn-info" value="Thanh Toán" style="width:100%;margin-top:10px;">
                <a href="bill.php" class="btn btn-info"  style="width:100%;margin-top:10px;">Xem các hóa đơn</a>
  </p>
